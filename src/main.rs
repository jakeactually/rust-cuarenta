...
use uno::{room_and_player, Cuarenta};
use user::User;

#[actix_web::main]
async fn main() -> std::io::Result<()> {
    let data = web::Data::new(Mutex::new(Cuarenta::new()));
...
async fn index(data: web::Data<Mutex<Cuarenta>>, session: Session) -> impl Responder {
    let player_id = session.get::<u32>("player_id").unwrap().unwrap_or(0);
    let users = &data.lock().unwrap().users;
    let default = User::new("", 0);
...
async fn new_room(
    ...
    context.rooms.insert(room_index, Room::new().push(user).clone());
    web::Json(NewRoomRes {
        room_id: context.room_index,
    })
}

fn get_user(context: &mut MutexGuard<Cuarenta>, form: web::Json<NewRoomReq>, session: Session) -> User {
...
async fn room(data: web::Data<Mutex<Cuarenta>>, req: HttpRequest, session: Session) -> impl Responder {
...
async fn state(
    ...
    subscribers
        .get_mut(&room_id)
        .unwrap()
        .push(uno::CuarentaSocket { session });
